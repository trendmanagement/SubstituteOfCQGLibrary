                ...
                if (contractListOfRollovers != null)
                {
                        ...
                        portfolioGroupCount++)
                    {
                        ...
                        for (int contractCount = 0; contractCount < contractListOfRollovers.Count; contractCount++)
                        {
                            ...
                            {
                                if (contractListOfRollovers[contractCount].contractType
                                    == OPTION_SPREAD_CONTRACT_TYPE.FUTURE)
                                {
                                    defaultPrice = contractListOfRollovers[contractCount].futurePriceUsedToCalculateStrikes;
                                }
                                else
                                {
                                    char typeSymbol = 'P';
                                    ...
                                    defaultPrice = OptionCalcs.blackScholes(typeSymbol,
                                        contractListOfRollovers[contractCount].futurePriceUsedToCalculateStrikes,
                                                contractListOfRollovers[contractCount].strikePrice,
                                                daysToExpiration / 365, riskFreeRate,
                                                impliedVol/100);
                                }
                            }
            ...
            for (int row = 0; row < gridViewSpreadGrid.Rows.Count; row++)
            {
                    ...
                    && gridViewSpreadGrid.Rows[row].Cells[(int)OPTION_PL_COLUMNS.MTH_AS_INT].Value != null)
                {
                    ...
                    while (expressionCnt < optionSpreadExpressionList.Count)
                    {
                        ...
                        if (currentContractType == optionSpreadExpressionList[expressionCnt].callPutOrFuture)
                        {
                            if (currentContractType == OPTION_SPREAD_CONTRACT_TYPE.FUTURE)
                            {
                                if (year == optionSpreadExpressionList[expressionCnt].futureContractYear
                                    && monthInt == optionSpreadExpressionList[expressionCnt].futureContractMonthInt)
                                {
                            ...
                            {
                                if (year == optionSpreadExpressionList[expressionCnt].optionYear
                                    && monthInt == optionSpreadExpressionList[expressionCnt].optionMonthInt
                                    && strike == optionSpreadExpressionList[expressionCnt].strikePrice)
                                {
                                    double optionTickSize = OptionSpreadManager.chooseOptionTickSize(
                                        optionSpreadExpressionList[expressionCnt].defaultPrice,
                                        instrument.optionTickSize,
